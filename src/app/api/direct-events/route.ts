...
export async function GET() {
  try {
    ...
    if (!env.DATABASE_URL) {
      ...
      return NextResponse.json({ error: 'Database connection string not defined' }, { status: 500 });
    }

    console.log('Direct API: Connecting to MongoDB with URL:', env.DATABASE_URL.substring(0, 20) + '...');
    ...
    if (mongoose.connection.readyState !== 1) {
      console.error('Direct API: MongoDB connection not ready, state:', mongoose.connection.readyState);
    ...
    console.log('Direct API: Available collections:', collections.map(c => c.name));
    ...
    if (!collections.some(c => c.name === 'events')) {
      ...
      return NextResponse.json({ error: 'Events collection does not exist' }, { status: 404 });
    }
    ...
    console.log(`Direct API: Found ${rawEvents.length} events directly from MongoDB`);

    // Log the raw events for debugging
    console.log('Direct API: Raw events sample:', rawEvents.slice(0, 2).map(e => {
      const { _id, ...rest } = e;
    ...
    const events = rawEvents.map(event => {
      ...
      return {
        ...
        description: event.description || event.desc || event.eventDescription || '',
        ...
        startDate: event.startDate || event.start_date || event.eventStart || new Date(),
        endDate: event.endDate || event.end_date || event.eventEnd || new Date(),

        // Use all possible category field names
        category: event.category || event.eventCategory || event.type || 'general',
        ...
        maxAttendees: event.maxAttendees || event.max_attendees || event.attendeeLimit || ['100'],

        // Use all possible creator field names
        createdById: event.createdById || event.created_by || event.userId || event.organizer || 'user-id',

        // Use all possible image field names
        image: event.image || event.eventImage || event.thumbnail || '',

        // Use all possible date field names
        createdAt: event.createdAt || event.created_at || event.dateCreated || new Date(),
        updatedAt: event.updatedAt || event.updated_at || event.dateUpdated || new Date(),
    ...
    try {
      ...
      console.error('Direct API: Error closing MongoDB connection:', closeError);
    }
  }
}
