...
export const GET = createApiRouteHandler<void>(
  async ({ query }): Promise<Response> => {
    try {
      ...
      console.log(
        "API route: Fetching events with conditions:",
        queryConditions,
      );
      ...
      try {
        console.log(
          "API route: Attempting to fetch events with mongoose model",
        );
        ...
        console.log(
          "API route: Found",
          events.length,
          "events with mongoose model",
        );

        // If no events found, try fetching all events
        if (events.length === 0) {
          console.log(
            "API route: No events found with filters, fetching all events",
          );
        ...
        if (events.length === 0) {
          console.log(
            "API route: No events found with mongoose, trying direct MongoDB access",
          );
          try {
            ...
            if (mongoose.connection && mongoose.connection.readyState === 1) {
              ...
              if (db) {
                ...
                console.log(
                  "API route: Found",
                  directEvents.length,
                  "events with direct MongoDB access",
                );
                events = directEvents;
              }
            } else {
              console.log(
                "API route: MongoDB connection not ready, state:",
                mongoose.connection?.readyState,
              );
            }
          } catch (directError) {
            console.error(
              "API route: Error with direct MongoDB access:",
              directError,
            );
          }
        }
