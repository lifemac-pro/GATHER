...
export async function GET() {
  try {
    ...
    let events = [];
    try {
      events = await Event.find({ endDate: { $gte: now } }).sort({
        startDate: 1,
      });
      console.log(`Direct API: Found ${events.length} upcoming events`);
    } catch (modelError) {
      console.error(
        "Direct API: Error fetching upcoming events with model:",
        modelError,
      );
    }
    ...
    if (finalEvents.length === 0) {
      try {
        console.log(
          "Direct API: No events found with mongoose, trying direct MongoDB access",
        );
        const mongoose = await connectToDatabase();
        const db = mongoose.connection.db;
        if (db) {
          // First try upcoming events
          let directEvents = await db
            .collection("events")
        ...
        console.error(
          "Direct API: Error with direct MongoDB access:",
          directError,
        );
      }
    }
