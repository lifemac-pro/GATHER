      ...
      {isLoading ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          ...
          {events?.map((event: any, index: number) => (
            <Card key={event.id || `event-${index}`} className="overflow-hidden">
              <EventImage src={event.image} alt={event.name || `Event ${index + 1}`} />
              <div className="p-6">
                  ...
                  <div>
                    ...
                    <div className="mt-2 space-y-1 text-sm text-gray-500">
                      <div className="flex items-center gap-2">
                        ...
                        <span>
                          {formatDate(event.startDate, "MMM d, yyyy 'at' h:mm a")}
                        </span>
                      </div>
                      {event.location && (
                        <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-2">
                    <span
                      className={cn(
                        ...
                        {
                          "bg-green-100 text-green-700": event.status === "published" || !event.status,
                    ...
                    >
                      {event.status ? (event.status.charAt(0).toUpperCase() + event.status.slice(1)) : 'Published'}
                    </span>
                  </div>
                </div>
