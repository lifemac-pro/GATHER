...
export const attendeeStatusEnum = ["registered", "attended", "cancelled", "waitlisted"] as const;
...
export const attendees = createTable(
  "attendee",
  {
    ...
    status: text("status", { enum: attendeeStatusEnum }).default("registered").notNull(),
    ...
    feedbackRequested: int("feedback_requested", { mode: "boolean" }).default(false),
...
export const events = createTable(
  "event",
  {
    ...
    allowWaitlist: int("allow_waitlist", { mode: "boolean" }).default(false).notNull(),
    autoConfirmWaitlist: int("auto_confirm_waitlist", { mode: "boolean" }).default(false),
    sendReminders: int("send_reminders", { mode: "boolean" }).default(true).notNull(),
    reminderDays: int("reminder_days").default(1),
    feedbackEnabled: int("feedback_enabled", { mode: "boolean" }).default(true).notNull(),
    autoRequestFeedback: int("auto_request_feedback", { mode: "boolean" }).default(true),
  },
  (table) => ({
