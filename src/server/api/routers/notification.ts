...
export const notificationRouter = createTRPCRouter({
  ...
  createNotification: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (input.isGlobal || !input.targetUsers || input.targetUsers.length === 0) {
        ...
        const result = await db.collection(NotificationCollection).insertOne(globalNotification);
      ...
      // If targetUsers is provided, send to specific users
        ...
        const result = await db.collection(NotificationCollection).insertMany(notifications);
  ...
  getAllNotifications: protectedProcedure
    .query(async ({ ctx }) => {
      ...
      const notifications = await db.collection(NotificationCollection).aggregate(pipeline).toArray();
