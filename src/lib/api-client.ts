...
class ApiClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      if (!response.ok) {
        const errorCode =
          responseData.error?.code || ErrorCode.INTERNAL_SERVER_ERROR;
        const errorMessage =
      ...
      if (error instanceof DOMException && error.name === "AbortError") {
        throw new AppError(ErrorCode.REQUEST_TIMEOUT, "Request timed out", 408);
      }
...
export const api = {
  ...
  events: {
    ...
    update: <T>(id: string, data: any) =>
      apiClient.put<T>(`/events/${id}`, data),
  ...
  attendees: {
    ...
    exportToCSV: <T>(params?: any) =>
      apiClient.get<T>("/attendees/export", { params }),
  },
  surveys: {
    submit: <T>(data: any) => apiClient.post<T>("/surveys", data),
    getByEvent: <T>(eventId: string) =>
      apiClient.get<T>(`/surveys/event/${eventId}`),
  },
  waitlist: {
    join: <T>(data: any) => apiClient.post<T>("/waitlist/join", data),
    getStatus: <T>(eventId: string) =>
      apiClient.get<T>(`/waitlist/status/${eventId}`),
  ...
  notifications: {
    ...
    markAsRead: <T>(ids: string[]) =>
      apiClient.post<T>("/notifications/mark-read", { notificationIds: ids }),
  ...
  user: {
    ...
    changePassword: <T>(data: any) =>
      apiClient.post<T>("/user/change-password", data),
  },
  analytics: {
    getStats: <T>(params?: any) =>
      apiClient.get<T>("/analytics/stats", { params }),
    getAttendanceData: <T>() => apiClient.get<T>("/analytics/attendance"),
    getDailyTrends: <T>(params?: any) =>
      apiClient.get<T>("/analytics/trends/daily", { params }),
    getMonthlyStats: <T>(params?: any) =>
      apiClient.get<T>("/analytics/trends/monthly", { params }),
  },
};
